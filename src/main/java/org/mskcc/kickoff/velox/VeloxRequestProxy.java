...
public class VeloxRequestProxy implements RequestProxy {
    ...
    private void addPoolRunsToSamples(Request request) {
        Set<Run> sampleRuns = request.getSamples().values().stream().flatMap(s -> s.getRuns().stream()).collect(Collectors.toSet());
...
//                pairing.setIsTumor(true);
    ...
    private void setRecipe(Request request, DataRecord dataRecordRequest) {
        ...
        try {
            samples = Arrays.asList(dataRecordRequest.getChildrenOfType(VeloxConstants.SAMPLE, user));
        ...
        try {
            List<Recipe> recipeList = recipes.stream().map(r -> Recipe.getRecipeByValue(r.toString())).distinct().collect(Collectors.toList());
        ...
        try {
            for (String pooledNormalPool : pooledNormalPools) {
                if (request.getPools().keySet().stream().anyMatch(p -> p.contains(pooledNormalPool))) {
                    ...
                    if(pool.isPresent()) {
                        Collection<Run> poolRuns = pool.get().getValue().getRuns();
    ...
    private void addPoolRunsToSample(Request request, Sample sample) {
        ...
        for (Pool pool : poolsWithCurrentSample) {
            for (Run run : pool.getRuns()) {
                if(!sample.getRuns().contains(run))
                    sample.addRun(run);
            }
        }
    }
