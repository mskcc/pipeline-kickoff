...
import org.mskcc.kickoff.roslin.validator.RequestFileValidator;
...
import static org.mskcc.kickoff.roslin.util.Utils.getJoinedCollection;
...
public class RequestFilePrinter extends FilePrinter {
    ...
    public void print(KickoffRequest request) {
        ...
        requestFileContents.append("RunID: ").append(Utils.getJoinedCollection(request.getRunIds(), ", ")).append("\n");
        ...
        if (request.getRequestType() == RequestType.RNASEQ) {
            requestFileContents.append("AmplificationTypes: ").append(Utils.getJoinedCollection(request.getAmpTypes(), ", " +
                    "")).append("\n");
            ...
            requestFileContents.append("Strand: ").append(Utils.getJoinedCollection(request.getStrands(), ", ")).append("\n");
        ...
        try {
            requestFileContents = new StringBuilder(Utils.filterToAscii(fileContents));
    ...
    private void addRerunReason(KickoffRequest request, StringBuilder requestFileContents) {
        if (request.getRunNumber() > 1) {
            String message = String.format("This project has been run before, rerun reason provided: %s", Arguments.rerunReason);

            if (StringUtils.isEmpty(Arguments.rerunReason)) {
                ...
                request.setRerunReason(Arguments.rerunReason);
            }
    ...
    private String getJoinedLibTypes(KickoffRequest request) {
        return Utils.getJoinedCollection(request.getLibTypes(), ",");
    }
