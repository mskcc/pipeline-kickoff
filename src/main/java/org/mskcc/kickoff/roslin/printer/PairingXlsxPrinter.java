...
class PairingXlsxPrinter extends FilePrinter {
    ...
    public void print(KickoffRequest request) {
        ...
        pairingInfo = Utils.addRowToSheet(wb, pairingInfo, new ArrayList<>(Arrays.asList(Constants.TUMOR, Constants
        ...
        for (String tum : pair_info.keySet()) {
            ...
            pairingInfo = Utils.addRowToSheet(wb, pairingInfo, new ArrayList<>(Arrays.asList(tum, norm)), rowNum, null);
            rowNum++;
        }

        for (String unmatchedNorm : missingNormalsToBeAdded) {
            String tum = "na";
            pairingInfo = Utils.addRowToSheet(wb, pairingInfo, new ArrayList<>(Arrays.asList(tum, unmatchedNorm)), rowNum,
                    null);
            rowNum++;
