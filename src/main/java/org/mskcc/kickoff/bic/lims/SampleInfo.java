...
public class SampleInfo {
     ...
     **/
    public SampleInfo(User apiUser, DataRecordManager drm, DataRecord rec, KickoffRequest request, Sample sample) {
        // Save logger
        ...
        try {
            ...
            drList.add(rec);
            CMOInfoMap = drm.getFieldsForChildrenOfType(drList, VeloxConstants.SAMPLE_CMO_INFO_RECORDS, apiUser);
            if (sample.isTransfer() && CMOInfoMap.get(0).size() == 0) {
                ...
                for (DataRecord parSam : parentSamples) {
                    List<DataRecord> cmoI = Arrays.asList(parSam.getChildrenOfType(VeloxConstants.SAMPLE_CMO_INFO_RECORDS, apiUser));
                    if (cmoI.size() > 0) {
                        drList.clear();
                        drList.add(parSam);
                        CMOInfoMap = drm.getFieldsForChildrenOfType(drList, VeloxConstants.SAMPLE_CMO_INFO_RECORDS, apiUser);
