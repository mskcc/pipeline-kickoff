...
public class ManifestFilePrinter implements FilePrinter  {
    ...
    public void print(KickoffRequest request) {
        ...
        try {
            ...
            try {
                ...
                String lastSpot;
                if (header.size() > Constants.MAX_HEADER_SIZE) {
                    int firstLetter = header.size() / Constants.MAX_HEADER_SIZE;
                    int remainder = header.size() - (Constants.MAX_HEADER_SIZE * firstLetter);
    ...
    public XSSFSheet addRowToSheet(XSSFSheet sheet, Map<String, String> map, int rowNum, KickoffRequest request) {
        try {
    ...
    public boolean shouldPrint(KickoffRequest request) {
        return (!request.getRequestType().equals(Constants.RNASEQ) && !request.getRequestType().equals(Constants.OTHER))
                && !(Utils.isExitLater() && !request.isInnovationProject() && !request.getRequestType().equals(Constants.OTHER) && !request.getRequestType().equals(Constants.RNASEQ));
    }
}
