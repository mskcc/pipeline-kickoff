...
import org.mskcc.kickoff.bic.domain.Patient;
import org.mskcc.kickoff.bic.domain.RequestSpecies;
...
public class FilesGenerator implements ManifestGenerator {
    ...
    public void generate(String projectId) {
        ...
        if (projectNameValidator.isValid(projectId)) {
            List<KickoffRequest> requests = requestProxy.getRequests(projectId);
    ...
    private void validateRequestExists(List<KickoffRequest> requests) {
        if (requests.size() == 0) {
    ...
    private void generate(KickoffRequest request) {
        try {
    ...
    private void addSamplesToPatients(KickoffRequest request) {
        for (Sample sample : request.getAllValidSamples().values()) {
    ...
    private void getManualOverrides(KickoffRequest request) {
    ...
    private void setNewBaitSet(KickoffRequest request) {
        String newBaitset;
    ...
    private Map<String, String> getProjectInfo(KickoffRequest request) {
    ...
    private void validateSpecies(KickoffRequest request, Sample sample) {
        try {
    ...
    private int getRunNumber(KickoffRequest request) {
