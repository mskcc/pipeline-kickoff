...
@Component
public class RecordNimblegenResolver implements NimblegenResolver {
    ...
    private DataRecord getLastValidRecord(List<DataRecord> nimbProtocols, List<Object> validities, String sampleId) {
        DEV_LOGGER.info(String.format("Nimblegen protocol records found for sample %s: %s", sampleId, nimbProtocols
                .stream().map(n -> n.getRecordId()).collect(Collectors.toList())));
