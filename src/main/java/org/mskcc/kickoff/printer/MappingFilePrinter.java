        ...
        try {
            ...
            for (KickoffRequest singleRequest : request.getRequests()) {
                ...
                for (SampleRun sampleRun : sampleRuns) {
                    Set<String> sample_pattern = new HashSet<>();
                    ...
                    if (files.length == 0) {
                        ...
                        for (File runDir : files) {
                            File requestPath = new File(runDir.getAbsoluteFile().toString() + "/Project_" + singleRequest);
                        ...
                        if (files.length == 0) {
                            String message = String.format("Could not find sequencing run folder that also contains request %s for Run ID: %s", singleRequest, runId);
                            Utils.setExitLater(true);
                            PM_LOGGER.log(Level.ERROR, message);
                            DEV_LOGGER.log(Level.ERROR, message);
                            request.setMappingIssue(true);
            ...
            if (mappingFileContents.length() > 0) {
                ...
                mappingFile = new File(String.format("%s/%s_%s", request.getOutputPath(), Utils.getFullProjectNameWithPrefix(request.getId()), mappingFileName));
